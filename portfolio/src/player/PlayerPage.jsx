...
import { ArrowLeft, Headphones, ListMusic, Pause, Play, Plus, SkipBack, SkipForward, Volume2, VolumeX } from 'lucide-react'
      ...
      <BorderGlow backgroundColor="#131216" glowColor="25 65 70" colors={['#b47764', '#873b50', '#c4b3a1']} glowIntensity={.6} borderRadius={20} className="player-glow">
        <section className="player-console" aria-label="音乐播放器">
          ...
          {source === 'netease' ? <NeteasePanel /> : <>
            ...
            <label className="progress-control"><span className="sr-only">播放进度</span><input type="range" min="0" max={Number.isFinite(music.duration) ? music.duration : 0} step="0.01" value={Math.min(music.time, music.duration || 0)} onChange={event => music.seek(Number(event.target.value))} /><span className="time-row"><i>{formatTime(music.time)}</i><i>{formatTime(music.duration)}</i></span></label>
            ...
            <div className="track-list"><span><ListMusic size={16} />播放列表 · {music.tracks.length} 首</span>{music.tracks.map((track, index) => <button key={track.id} className={index === music.index ? 'active' : ''} aria-pressed={index === music.index} onClick={() => music.choose(index)}><i>{String(index + 1).padStart(2, '0')}</i><b>{track.title}</b><small>{track.local ? '本地' : '氛围片段'}</small></button>)}</div>
          </>}
        </section>
      </BorderGlow>
    </main>
